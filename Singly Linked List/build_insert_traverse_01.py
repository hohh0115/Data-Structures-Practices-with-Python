...
def insert_at_point(begin_node, insert_count, insert_index, loop_count):
    """
    插入任意位置
    :param begin_node: linked list第幾個位置的前一個節點
    :param insert_count: 插入多少節點
    :param insert_index: 插入第幾個位置
    :param loop_count: 使用者操作次數
    :return:
    """
    for i in range(1, insert_count + 1):
...
while make_list is not True:
    ...
    if select == 1:
        ...
        point = head
        try:
            while j < insert_index: # 第insert_index位置的前一個節點就可以，第insert_index位置的節點用其前一個節點的指標做指向
                point = point.next
                j += 1
        ...
        insert_at_point(point, count, insert_index, loop_count)
    elif select == -1:
        make_list = True
    else:
        print('看不懂')
